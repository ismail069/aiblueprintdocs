...
- a **review note** is a visible assumption, decision, risk, or warning that deserves your attention; it can be accepted and tracked and does not automatically mean the document is broken;
...
The Review Notes number counts the currently visible review-note items across the Blueprint. Open the related document to read each item. Signing a document accepts the notes visible in that version; it does not silently remove them.

## Reported Problems in App Structure

**Reported Problems** are unresolved consistency findings returned with the App Structure, such as a navigation link that references a feature missing from the map. The badge is the total number of findings in the list, not the number of feature cards. These findings are not automatically resolved just because the canvas rendered successfully.

Review every finding before approval. Correct the product decision or source context when necessary, then use the Assistant proposal and regenerate the affected document. A repaired finding no longer appears in the current unresolved list.
